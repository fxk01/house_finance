  ...
  <div class="commission">
    <div v-if="this.$cookie.get('isAuth') === 'true'" class="lh-1">
      ...
      <ul class="flex flex-align-center flex-justify-between container2">
        ...
        <li class="flex-1" v-on:click="showMoney = !showMoney">金额详情</li>
        <li class="flex-1" v-on:click="val = withdrawalData['available']">全部提现</li>
      </ul>

      <div class="wrap-cost">
        <div>
          <p v-if="!defaultBut">综合手续费：0</p>
          <p v-if="defaultBut">综合手续费：{{(val / 1.071 * 0.071).toFixed(2)}}</p>
          <p>综合手续费=通道费用+个人所得税</p>
        </div>

        <p class="last-text" v-if="!defaultBut">到账金额：{{val}}</p>
        <p class="last-text" v-if="defaultBut">到账金额：{{(val / 1.071).toFixed(2)}}</p>
      </div>
